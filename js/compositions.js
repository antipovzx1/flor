const womanList = document.getElementById('itemList');
                ...
                if (item.flor.includes(filter[0]) && item.wrapColor.includes(filter[1])) {
                    womanList.innerHTML += `
                        <div class="section-card">
                            ...
                            <button data-id=${item.id} data-img="${item.img}" data-prace=${item.prace} data-category="сompositions" data-flor="${item.flor}" data-wrapColor="${item.wrapColor}" onclick="addToCart(this)">В корзину</button>
                        </div>
                    `;
                    countItem++;
                }
...
fetch('db/categoryesj.json')
    ...
    .then(data => {
        ...
        selectColor = data.corsinaColor[0];
