    ...
    fetch('db/food.json')
        ...
        .then(data => {
            ...
            for (let i = 0; i < data.totalCount; i++) {
                item = data.item[i];
                console.log(item)
                if (item.food.includes(filter[0]) && item.wrapColor.includes(filter[1])) {
                    womanList.innerHTML += `
                        <div class="section-card">
                            <img src="${item.img}" alt="">
                            <span>${item.prace} р</span>
                            <button data-id=${item.id} data-img="${item.img}" data-prace=${item.prace} data-category="food" data-food="${item.food}" data-wrapColor="${item.wrapColor}" onclick="addToCart(this)">В корзину</button>
                        </div>
                    `;
                    countItem++;
                }
            ...
            if(countItem == 0){
                womanList.innerHTML = `<h2 class="pustoList">Товаров с асорти - <span>${filter[0]}</span> и цветом обёртки - <span>${filter[1]}</span></h2>`;
            }
        })
}
...
fetch('db/categoryesj.json')
    .then(response => { return response.json() })
    .then(data => {
        selectFlor = data.food[0];
        selectColor = data.foodColor[0];
